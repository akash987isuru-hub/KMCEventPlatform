...
public static class OrganizerTicketPdfGenerator
{
    public static byte[] Generate(OrganizerBookingViewModel booking)
    {
        ...
        content.AppendLine($"({Escape("SPARKLING EVENTS KANDY TICKET")}) Tj");

        AddLine(content, "A curated event experience from the heart of Kandy", 11, false, 26);
        ...
        AddLine(
            content,
            $"Schedule: {booking.EventDate:dd MMM yyyy} {booking.StartTime.ToString(@"hh\:mm", CultureInfo.InvariantCulture)} - {booking.EndDate:dd MMM yyyy} {booking.EndTime.ToString(@"hh\:mm", CultureInfo.InvariantCulture)}",
            11,
            false,
            22);
        ...
        AddLine(content, $"Payment: {booking.PaymentStatus} / {booking.CardDisplay}", 11, false, 22);
        ...
        AddLine(content, "Keep this reference ready at the welcome desk.", 11, false, 34);
        AddLine(content, "Questions: isuru@sparklingeventskandy.lk | +94 76 845 2190", 9, false, 20);
